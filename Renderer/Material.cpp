...
bool Metal::Scatter(const Ray& ray, const RaycastHit& hit, color3& attenuation, Ray& scattered) const
{
    ...
    reflected = reflect(glm::normalize(ray.direction), hit.normal);
...
bool Emissive::Scatter(const Ray& ray, const RaycastHit& hit, color3& attenuation, Ray& scattered) const
{
    return false;
}
