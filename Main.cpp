...
int main(int, char**)
{
	...
	//Camera camera({ 0, 4, 4 }, { 0, 0, 0 }, { 0, 1, 0 }, fov, width / (float)height);
...
void InitScene01(Scene& scene)
{
	...
	//objects
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ -1.25f, 0.45f, -1.0f }, 0.5f, std::make_unique<Lambertian>(color3{ 0, 1, 0 })));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 1.25f, 0.45f, -1.0f }, 0.5f, std::make_unique<Metal>(color3{ 1, 1, 1 }, 0.3f)));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 0.0f, 0.45f, -1.0f }, 0.5f, std::make_unique<Dielectric>(color3{ 1, 1, 1 }, 2.42f)));
